  ...
  if(!token) {
    ...
    .then((data) => {
      ...
      if(!data.data.file){
        document.querySelector(
          "#file"
        ).innerHTML += `<a class="w-full px-4 text-base font-normal outline-none text-black py-2 hover:bg-purple-700" href="${BASE_URL}${data.data.file}"></a>`;
      }
      
      document.querySelector(
        "#url"
      ).innerHTML += `<a class="w-full px-4 text-base font-normal outline-none text-black py-2 hover:bg-purple-700" href="${data.data.url}">${data.data.url}</a>`;
      document.querySelector(
        "#deadline"
